...
INT_PTR DlgSettings::OnInit(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	_injectionType.Add(L"手动 map", Manual);

	_injectionType.Add(L"内核注入 (创建线程)", Kernel_Thread);
	_injectionType.Add(L"内核注入 (APC)", Kernel_APC);
	_injectionType.Add(L"内核注入 (手动 map)", Kernel_MMap);
...
DWORD DlgSettings::HandleDriver(uint32_t type)
{
	...
	if (!NT_SUCCESS(status))
	{
		...
		if (status == STATUS_INVALID_IMAGE_HASH)
		{
			...
			if (Message::ShowQuestion(_hwnd, L"是否要关闭驱动签名验证?"))
			{
				STARTUPINFOW si = { 0 };
				...
				{
					Message::ShowWarning(_hwnd, L"驱动签名验证已经关闭,重新启动才能生效。");
				...
				{
					Message::ShowError(_hwnd, L"关闭驱动签名验证失败");
					status = LastNtStatus();
				}
